...
// прерывание через 1 ms, т.е. частота таймера = 500 Hz, T = 2 ms
// Вызов из stm32f1xx_it.c (TIM1_UP_IRQHandler)
//**********************************************************************************
void TicTimers( void ) {

    uint8_t index;
    
    for ( index = 0; index < ( sizeof( timers )/sizeof( timers[0] ) ); index++ ) 
        if ( timers[index] )
            timers[index]--;
 }
...
void LedIndicator( void ) {
    ...
    if ( led_mode == LED_MODE_ON ) {
        HAL_GPIO_WritePin( LED_RECV_GPIO_Port, LED_RECV_Pin, GPIO_PIN_RESET );
        return;
       } 
    if ( led_timer-- )
        return;
    state = HAL_GPIO_ReadPin( LED_RECV_GPIO_Port, LED_RECV_Pin );
    ...
    HAL_GPIO_TogglePin( LED_RECV_GPIO_Port, LED_RECV_Pin );
